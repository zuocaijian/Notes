### 一、起因
1. 之前一个比较老的项目中，是使用Activity的名字作为tag来标识网络请求的。在Activity的onDestroy回调中根据这个标识取消所有的网络请求。但是在部分页面，出现了比较奇怪的问题：从Activity A打开Activity B，然后finish掉Activity B回到Activity A，这时候再次打开Activity B，Activity B中的网络请求会出现概率性无回调导致页面加载失败，且毫无规律可循。后经过仔细排查，原来是第一次打开的Activity B实例onDestroy回调延时造成的。也就是说当第二次打开的Activity B实例启动后正在请求网络的时候，由于延时，第一次打开的Activity B实例的onDestroy回调正好被执行，取消了全部以Activity B名字作为tag的网络请求，当然会导致第二次打开的Activity B实例中网络请求无回调了。(此处暂不讨论这种取消网络请求的不合理性)
...
3. 之前由于项目开发周期紧，碰到问题的时候并没有时间深究，为了赶工，只是采取替代方案快速解决/绕开了存在的问题。这段时间需求少了一点，开始对项目中的一些重点页面做性能分析和优化。当对一个复杂的列表页和详情页进行检查的时候，再一次发现了onStop和onDestroy回调延时的问题。简单分析无果，加之又回想起那些被未知原因的bug支配的恐惧，于是下定决心，趁着现在有空来深入挖掘下回调延时的背后究竟隐藏着哪些不为人知的秘密。
### 二、原因/结论
> Activity调用流程：打开Activity A -> 打开Activity B -> 关闭Activity B，回到Activity A

1. 回调延时：
    由于**要关闭的Activity B或者将要打开的Activity A往主线程的MessageQueue中连续不断的post了大量的msg**，导致主线程一直在不断的进行消息循环处理这个消息而没有得到停息。因此，ActivityThread中的Idler.queueIdle方法没有被调用的机会，App侧也就不能向ActivityManagerService(AMS)发起IPC告知自己有空闲时间来处理AMS侧的任务，也就是AMS向App侧发起IPC来进行Activity B的销毁的正常流程被阻断了。所以，finish Activity B后，onDestroy不会被及时回调。具体延时多久，要看主线程中堆积的msg什么时候处理完，也就是说要看主线程什么时候闲下来。
2. 延时10s：
    除正常流程外，Android系统另行安排了一套流程来保证即使正常流程被阻断以后，Activity B还是能被销毁。这个流程就是用来预防正常流畅中App不会闲下来出来AMS侧的IPC任务而设计的(*源码注释：Schedule an idle timeout in case the app doesn't do it for us*)。流程具体为：在关闭Activity B返回Activity A的时候，当AMS侧发起IPC通知App侧的Activity A进行resume之后，便会向AMS自己的主线程发送一个msg，该msg延时10s后执行，该msg中任务的处理方为ActivityStackSupervisor#ActivityStackSupervisorHandler，任务的具体内容与正常流程App段空闲时需要执行的任务内容一致，当然也包括销毁Activity B。这样，Activity B的onDestroy方法也就在延时10s后被调用了。
### 三、问题复现
...
```
代码非常简单。我们知道，view.post()方法底层就是往主线程的消息队列MessageQueue发送消息(这里这么写只是为了少写两行代码)，因为整段代码逻辑就是，在SecondActivity中不断向主线程post空的msg，来模拟复杂页面中不停的重绘、数据流处理、帧动画处理等。
测试流程：我们首先打开FirstActivity，然后再FirstActivity中通过StartActivity方法来打开SecondActivity，接着我们按下回退键，关闭SecondActivity回到FirstActivity。如果我们打印出FirstActivity和SecondActivity，就不难发现，按下回退键后，在FirstActivity的onResume方法被调用后，SecondActivity的onStop和onDestroy方法并没有紧接着被回调，而是在恰好10s后才被回调。
### 四、源码探究
既然出现了问题，当然要想办法弄清楚才行。于是我们熟练的打开百度，输入onDestroy回调延时，随便找两篇博客看看也就知道答案了。
本文分析到此为止。
开个玩笑，要是常规操作能解决问题，我当然也就不用多次一举再来写这么一篇问题分析的博文了。这里随便贴几篇百度搜到的博文：
...
看完很绝望有没有啊。虽然很多博文虽然都写着原因分析，但完全没有适用性。给出的解决方案也是要么绕开这个坑，要么明着告诉你不要在onDestroy回调中去做balabala...。既然百度不行，那咱Google一下吧...Google好像也不太好使(或者说咱用的不好?)...还是不行，去stackoverflow看看，鉴于咱这英文阅读速度和搜出来的质量，在换了几个关键词之后也就放弃了。得，还是自己动手，丰衣足食吧。
首先从内存泄漏得角度考察了一番，什么LeakCanary、Android Profile和大杀器MAT，通通祭出来。结果闹了半天，仍然没有什么重大发现。看来是个棘手得问题，真叫人头秃啊，差点泄气要放弃了。可是留下这个问题，又让人恨得牙痒痒，甚至就此留下阴影，成为以后职业生涯修炼时走火入魔的导火索。哎，既然捷径都走不通，那看来只能笨办法，源码分析走起，我倒要看看系统这个鬼究竟搞了什么骚操作。
我们就以Activity的finish方法作为入口，一步一步，深入分析其流程。
本次分析，**源码基于Android 9.0**。其中会有Binder的进程间通信和Handler的进程内线程间通信相关知识，因为不是分析重点，一笔带过。另外onStop方法延迟调用与onDestroy方法延迟调用的原因有关联，分析流程也基本一致，所以这里以分析onDestroy方法延迟为主线。
```
// in Activity.java
...
```
为了使逻辑看起来更清晰，这里删减了一部分源码，根据流程，调整了方法的排列顺序，并对部分较为重要的源码进行了简单注释。整个流程调用链为：在AMS中发起调用finishActivity方法，进入ActivityStack中，依次调用 requestFinishActivityLocked -> finishActivityLocked -> finishActivityLocked -> startPausingLocked。在第一个finishActivityLocked方法中，我们详细注释了pauseImmediately参数的意义。同时，在startPausingLocked方法中，我们我们看到了这么一句代码：
```
mService.getLifecycleManager().scheduleTransaction(prev.app.thread, prev.appToken,
                        PauseActivityItem.obtain(prev.finishing, userLeaving,
                                prev.configChangeFlags, pauseImmediately));
```
其实是AMS通过Binder与App侧进行IPC，在App侧ActivityThread中的ApplicationThread作为通信接受方，然后再通过Handler进行进程间通信，通知ActivityThread中的H(集成Handler)来处理消息。其中TransactionExecutor作为真正的执行者，统一处理消息中所携带的任务。处理的内容即为执行PauseActivityItem中的postExecute方法，
```
...
public class PauseActivityItem extends ActivityLifecycleItem {
    ...
    public void postExecute(ClientTransactionHandler client, IBinder token,
        ...
        try {
            ...
            // 接受方是ActivityManagerService的activityPaused方法被调用
...
```
PauseActivityItem一共干了两件事，首先是回调当前Activity的onPause，然后发起IPC，告诉AMS可以执行后续流程了。我们ActivityManagerService来看看activityPaused方法：
```
// in ActivityManagerService
public class ActivityManagerService extends IActivityManager.Stub
        implements Watchdog.Monitor, BatteryStatsImpl.BatteryCallback {
...
public class ResumeActivityItem extends ActivityLifecycleItem {
    ...
    public void postExecute(ClientTransactionHandler client, IBinder token,
        ...
        try {
            ...
            // 接受方是ActivityManagerService的activityResumed方法被调用
...
```
ResumeActivityItem也就干了两件事，首先是回调下一个要显示的Activity的onResume，然后发起IPC，告诉AMS可以执行后续流程了。我们ActivityManagerService来看看activityResumed方法：
```
// in ActivityManagerService
public class ActivityManagerService extends IActivityManager.Stub
        implements Watchdog.Monitor, BatteryStatsImpl.BatteryCallback {
...
```
关键的代码我已经注释起来了，总而言之就是向App侧主线程的消息队列注册一个空闲处理器，以便主线程的消息队列中的任务执行完时处理一些低优先级的任务。这个空闲处理器的调用是一次性，也就是说调用一次之后就不会再被调用了。这个所谓的低优先级任务，其实就是向AMS侧发起IPC，告诉AMS，App闲下来了。进入ActivityManagerService，看看activityIdle方法：
```
//in ActivityManagerService
public class ActivityManagerService extends IActivityManager.Stub
        implements Watchdog.Monitor, BatteryStatsImpl.BatteryCallback {
...
public class ActivityStackSupervisor extends ConfigurationContainer implements DisplayListener,
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        //还记得前面我们分析的Activity销毁流程，
        //在Activity#finishCurrentActivityLocked方法中，眼看就要成功，却因条件不符功亏一篑的地方吗？
...
2. onStop和onDestroy回调为什么会延时
    前面我们完整分析了了finish后，回调onStop和onDestroy的正常流程。这中间经历了多次App侧和AMS侧的进程间通信，以及两端各自进程内通信。正常流程中较为关键的一点是，新的Activity在resume后，App侧的主线程空闲下来才会通知AMS执行后续流程，将关闭的Activity销毁。那要是新的Activity在resume后，主线程一直在循环处理MessageQueue堆积的消息呢？很显然，要是这样的话，通知AMS执行后续流程自然被延迟了，因而，Activity的onStop和onDestroy生命周期回调自然被延迟了。
3. 延时10s是为什么
    作为一个健壮的操作系统，当然要有一定的容错机制，不能说因为主线程一直忙，就不去销毁/回收已经死亡的Activity。要不然新手开发者开发的App就因为内存泄漏分分钟玩死系统了。那么这个容错机制是怎么设计的呢？
...
经历这一波分析，也算是对Framework层的AMS有了更清晰的理解了。
也算是给自己一个交代吧，完美！下班~
